...
public class DBigOpt2015UpdateDataByKafka
        extends AbstractDBigOpt2015UpdateDataByKafka {
    ...
    public DBigOpt2015UpdateDataByKafka(String instanceName) {
    ...
    public void evaluate(ContinuousIndividual solution) {
        ...
        for (int i = 0; i < dTypeG; i++) {
            ...
            for (int j = 0; j < icaComponent.get(0).size(); j++) {
                s1Temp.add(solution.getVariableValue(
                        i * (icaComponent.get(0).size()) + j));
            }
            s1.add(s1Temp);
        }
        ...
        for (int i = 0; i < icaComponent.size(); i++) {
            for (int j = 0; j < icaComponent.get(i).size(); j++) {
                sum += Math.pow(icaComponent.get(i).get(j) - s1.get(i).get(j),
                                2);
            }
        }
        ...
        solution.setObjectiveValue(0, obj1);
        solution.setObjectiveValue(1, obj2);
    }

    public void loadData(String problemId, String fName, int dType,
            int dLength) {
        List<List<Double>> list;
        String fileName = "/datasets/" + problemId + fName;
        ...
        if (fName.equals("X.txt")) {
            ...
            throw new RuntimeException("Wrong name: " + fName);
        }
    }
    ...
    List<List<Double>> correlation(List<List<Double>> list1,
    ...
    List<List<Double>> multiplyWithOutAMP(List<List<Double>> list1,
            List<List<Double>> list2) {
        List<List<Double>> c;
        List<Double> cTemp;
        ...
        for (int row = 0; row < list1.size(); row++) {
            for (int col = 0; col < list2.get(row).size(); col++) {
                for (int inner = 0; inner < list1.get(0).size(); inner++) {
                    ...
                    c.get(row).set(col, val + list1.get(row).get(inner) *
                            list2.get(inner).get(col));
                }
            }
        }
        return c;
    ...
    public List<List<Double>> getIcaComponent() {
        return icaComponent;
    }
    ...
    public void setIcaComponent(List<List<Double>> icaComponent) {
        this.icaComponent = icaComponent;
    }

    public void setMixed(List<List<Double>> mixed) {
        this.mixed = mixed;
    }

    public List<List<Double>> getMixed() {
        return mixed;
    }
